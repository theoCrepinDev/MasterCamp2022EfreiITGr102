...
const getCandidtascand = (idCandidat) => {
    return new Promise((resolve, reject) => {
        db.query("SELECT ID_candidat, Nom_candidat, Prénom_candidat, Description_candidat, Photo_candidat, Programme_candidat FROM candidat ORDER BY Nombre_vote DESC LIMIT 3;", async (err, resultRecupCandidats) => {
            if(err) throw err;
            if(resultRecupCandidats.length < 0){
                reject({
                    message : "Aucun candidat associé à ce suffrage affichage impossible"
...
const getnbvotes = (idSuffrage) => {
    return new Promise((resolve, reject) => {
        db.query("SELECT Nombre_vote FROM candidat ORDER BY Nombre_vote DESC LIMIT 3; " , async (err, resultnb) => {
            console.log("Nombre des votes récupéré");
            resolve(resultnb);
...
const getnbvotesCand = (idCandidat) => {
    return new Promise((resolve, reject) => {
        db.query("SELECT Nombre_vote FROM candidat ORDER BY Nombre_vote DESC LIMIT 3; " , async (err, resultnb) => {
            console.log("Nombre des votes récupéré");
            resolve(resultnb);
...
router.get('/candidat/:CNI', async (req, res) => {
    //on récupère les info des suffrage qui corresponde à l'user
    db.query("SELECT * FROM candidat ORDER BY Nombre_vote DESC LIMIT 3;", async (err, resultRecupCandidat) => {
        if(err) throw err;
        //on regarde si il y a au moins un suffrage
        if(resultRecupCandidat.length <= 0){
            res.status(204).json({message : "L'utilisateur n'est éligible à aucun vote..."})
        }else{
            
            const idCandidat = resultRecupCandidat[0].ID_candidat;
            console.log(idCandidat)
            var candidat = {
                ID_candidat: resultRecupCandidat[0].ID_candidat,
                Nom_candidat: resultRecupCandidat[0].Nom_candidat,
                Prénom_candidat: resultRecupCandidat[0].Prénom_candidat,
                Description_candidat: resultRecupCandidat[0].Description_candidat,
                Nombre_vote: resultRecupCandidat[0].Nombre_vote,
                Photo_candidat: resultRecupCandidat[0].Photo_candidat,
                Programme_candidat: resultRecupCandidat[0].Programme_candidat,
                ID_suffrage: resultRecupCandidat[0].ID_suffrage,
                candidats : [],
                nombre_Votants : []
            }
            //on récupère les candidats associés
            getCandidtascand(idCandidat)
                .then((response) => {
                    candidat.candidats = response
                    //on récupère le nombre de votant
                    getnbvotesCand(idCandidat)
                        .then((response) => {
                            candidat.nombre_Votants = response
                            console.log(Object.values(JSON.parse(JSON.stringify(response[0]))))
